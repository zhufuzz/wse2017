...
class Slave(wx.Frame):
    def __init__(self):
        ...
        self.copyClipboard = wx.Button(self.panel, label=u'查看剪贴板')
        ...
        self.Bind(wx.EVT_BUTTON, self.onCopy, self.copyClipboard)
        ...
        self.gridBagSizerAll.Add(self.send, pos=(6, 2),
                            flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL,
                            span=(1, 1), border=5)
        self.gridBagSizerAll.Add(self.copyClipboard, pos=(6, 3),
                                 flag=wx.ALL | wx.ALIGN_CENTER_VERTICAL,
                                 span=(1, 1), border=5)
    ...
    def onCopy(self, event):
        ...
        if server != -1:
            slave = self.serverList[server]
            self.masterSocket = MasterSocket(SERVER, PORT, 'copyClipboard', commandType='advancedFunction', to=slave)
        else:
            print u'请先选择被控端。'
            return None
        self.masterSocket.setDaemon(True)
    ...
    def onScreen(self, event):
        ...
        if server != -1:
            slave = self.serverList[server]
            self.masterSocket = MasterSocket(SERVER, PORT, 'printScreen', commandType='advancedFunction', to=slave)
        else:
            print u'请先选择被控端。'
            return None
        self.masterSocket.setDaemon(True)
        self.masterSocket.start()
        import thread
        thread.start_new_thread(self._changeScreen, ())
    ...
    def _changeScreen(self):
        time.sleep(5)
        if os.path.exists('screenshots.jpg'):
            try:
                img = wx.ImagSe(r'screenhots.jpg', wx.BITMAP_TYPE_ANY).Scale(300, 200)
    ...
    def _removeMaster(self):
        self.lock.acquire()
        if 'master' in self.serverList:
            self.serverList.remove('master')
        self.lock.release()
