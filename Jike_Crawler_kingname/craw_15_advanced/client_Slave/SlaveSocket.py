            ...
            while not self.stop:
                if not self.connected:
                    ...
                    try:
                        ...
                        if len(data):
                            result = self.analysisCommand(data)
                            if result:
                                print u'调用send to server'
    ...
    def analysisCommand(self, data):
        ...
        if data:
            ...
            if command == 'runaway':
                ...
                result = self.executor.execute(commandType, command)
                print u'运行的命令是: %s done!' % command
                return result

    def sendToServer(self, result):
        try:
            self.sock.sendall(self.generateJson(result))
        except Exception, e:
            print u'向服务器发送命令失败: %s' % str(e)

    def generateJson(self, info):
        returnInfo = {'from': 'slave', 'info': info}
        return json.dumps(returnInfo) + '#finished#'
