...
class ScreenShotter:
    ...
    def saveImg(self):
        ...
        ctypes.memmove(ctypes.pointer(bitmapInfoHeader), data, self.SIZEOF_BITMAPINFOHEADER)

        if bitmapInfoHeader.biCompression != BI_BITFIELDS:  # RGBA?
            print('insupported compression type {}'.format(bitmapInfoHeader.biCompression))
            sys.exit(1)

        bitmapFileHeader = BITMAPFILEHEADER()
        ctypes.memset(ctypes.pointer(bitmapFileHeader), 0, self.SIZEOF_BITMAPFILEHEADER)  # zero structure
        bitmapFileHeader.bfType = ord('B') | (ord('M') << 8)
        bitmapFileHeader.bfSize = self.SIZEOF_BITMAPFILEHEADER + len(data)  # file size
        SIZEOF_COLORTABLE = 0
        bitmapFileHeader.bfOffBits = self.SIZEOF_BITMAPFILEHEADER + self.SIZEOF_BITMAPINFOHEADER + SIZEOF_COLORTABLE

        bmp_filename = 'clipboard.bmp'
        print bmp_filename
        with open(bmp_filename, 'wb') as bmp_file:
            bmp_file.write(bitmapFileHeader)
            bmp_file.write(data)
